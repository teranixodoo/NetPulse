...
async def download_backup(
    ...
    """Stáhne soubor zálohy. Soubor musí existovat na disku."""
    ...
    if not filepath.exists():
        raise HTTPException(
            status_code=404,
            detail=f"Soubor zálohy neexistuje na disku: {filepath}"
        )
...
async def delete_backup(
    ...
    """Smaže záznam zálohy z DB a soubor z disku."""
    record = await db.get_backup_by_id(pool, backup_id)
    if not record:
        raise HTTPException(status_code=404, detail="Záloha nenalezena")
    ...
    if filepath.exists():
        try:
            ...
            log.warning(f"Nelze smazat soubor zálohy {filepath}: {e}")
