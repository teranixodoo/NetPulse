...
28. [3D trasování kabelů](#28-3d-trasování-kabelů)
...
Modul **Topologie sítě** (`/topology`) umožňuje vizuální dokumentaci fyzické a logické síťové infrastruktury na interaktivní mapě.

### 27.1 Přehled funkcí

| Funkce | Popis |
|---|---|
| Kabelové trasy | Fyzické trasy kabelů kreslené na mapě |
| Logické spoje | Propojení lokací/zařízení (optika, wireless, metalika) |
| Polygony budov | Barevné obrysy budov s definicí pater |
| 3D zobrazení | Interaktivní 3D model budovy s patry a kabely |
| Průchozí body | Evidence montážních krabic, patch panelů, průchodů |

### 27.2 Vrstvy mapy

V levém panelu lze zapínat/vypínat vrstvy:
- **Lokace** — hierarchické markery lokací (budovy, patra, waypoints)
- **Polygony budov** — barevné obrysy budov a bloků
- **Kabelové trasy** — fyzické 2D trasy kabelů
- **Logické spoje** — logická propojení

### 27.3 Hierarchie lokací v Topologii

Lokace jsou zobrazeny jako barevné kruhy dle hloubky:
- 🔵 Modrá = budova/region (hloubka 0)
- 🟠 Oranžová = sub-budova/blok (hloubka 1)
- 🟣 Fialová = patro/místnost (hloubka 2+)

**Plný kruh** = vlastní GPS. **Přerušovaný** = zděděné GPS.

Klik na badge (číslo u markeru) = rozbalit/sbalit podřízené lokace.
Lokace typu **region** jsou automaticky rozbaleny.

### 27.4 Varianta B — Budova s bloky

Pro budovu rozdělenou na části s různým počtem pater:

```
V1 - Budova (building)          ← kontejner bez polygonu
  ├── V1.1 Levý blok (building) ← vlastní polygon + patra
  └── V1.2 Pravý blok (building)← vlastní polygon + patra
```

**Postup:**
1. Vytvoř sub-budovy v Evidenci lokací (type=building, parent=V1)
2. V Topologii rozbal V1 → zobrazí se V1.1 a V1.2
3. Pro každý blok nakresli polygon a nastav patra

### 27.5 Kabelové trasy (2D)

1. Klikni **Kreslit trasu kabelu**
2. Klikej body trasy na mapě
3. Dvojklik nebo **Potvrdit** = dokončit
4. Formulář: název, typ, medium, délka, lokace A/B, stav

### 27.6 Logické spoje

1. Klikni **Přidat spoj**
2. Klikni na lokaci A (zelený kruh)
3. Klikni na lokaci B (červený kruh)
4. Formulář: typ spoje, zařízení, interface, (wireless: frekvence, azimut)

### 27.7 Polygony budov

1. Klikni **Nakreslit polygon**
2. Klikni na marker budovy nebo bloku
3. Klikej body obrysu budovy
4. Dvojklik nebo **Dokončit** = uzavřít
5. Formulář: název, patra, výška/patro, barva

**Menu polygonu** (klik na polygon):
- 🏗️ **Zobrazit ve 3D** — otevře 3D model budovy
- ⚙️ **Vlastnosti** — editace + vazba pater na lokace
- ✏️ **Upravit tvar** — (připraveno)
- 🗑️ **Smazat**

### 27.8 Vlastnosti polygonu — vazba pater

V sekci **Patra — vazba na lokace** lze každé patro provázat s podřízenou lokací:
- Dropdown = výběr existující floor lokace
- **+ Vytvořit** = vytvoří novou lokaci patra (type=floor, dědí GPS)
- Ukládá se jako `floor_location_ids: {"0": id, "1": id, ...}`

### 27.9 Průchozí body (Waypoints)

Evidence průchozích bodů kabelových tras v hierarchii lokací.

**Přidání waypointu:**
1. Evidence lokací → Přidat lokaci
2. Typ: **Průchozí bod**
3. Vyber nadřazenou lokaci (patro budovy)
4. Vyber typ průchozího bodu:

| Typ | Ikona | Popis |
|---|---|---|
| `junction_box` | 🟫 | Montážní/rozbočovací krabice |
| `patch_panel` | 🔲 | Patch panel |
| `conduit` | ⬜ | Kabelový žlab / trubka |
| `wall_penetration` | 🔵 | Průchod stěnou |
| `floor_penetration` | 🔴 | Průchod stropem/podlahou |
| `outlet` | 🔌 | Zásuvka / vývodka |

5. Zadej **výšku nad podlahou (m)** — výchozí 0 = podlaha, 2.5 = průchod stropem

Waypointy se zobrazují v 3D pohledu budovy jako barevné ikony na správné výšce.

---

## 28. 3D trasování kabelů

### 28.1 Přístup

V Topologii klikni na polygon budovy → **🏗️ Zobrazit ve 3D**.

### 28.2 Rozhraní 3D pohledu

**Levá část — interaktivní 3D mapa (MapLibre GL):**
- Každé patro = barevný blok
- Tažení myší = rotace
- Kolečko = zoom
- Ctrl + tažení = náklon

**Pravý panel:**
- **Patra** — klik = zvýrazní patro, přepne výšku při kreslení
- **Kabely** — seznam kabelů v budově s barevnými indikátory
- **Průchozí body** — waypoints v budově
- **Zařízení** — zařízení přiřazená k patrum

### 28.3 Kreslení 3D trasy kabelu

1. Klikni **✏️ Kreslit trasu kabelu**
2. Zvol aktivní **patro** v toolbaru (nastaví výšku pro body trasy)
3. Klikej body trasy na půdorysu budovy
   - Klik na **waypoint marker** = přichytí bod k waypointu (název, výška)
   - Klik na volné místo = bod s výškou středu patra
4. Přepni patro = výška dalších bodů se změní
5. Klikni **✓ Dokončit** (min. 2 body)
6. Vyplň formulář:
   - Název kabelu (povinné)
   - Typ (Optika / UTP / Koax / Ostatní)
   - Bod A / Bod B = provázání s lokacemi pater
7. Klikni **Uložit**

### 28.4 Uložení a synchronizace

Po uložení je kabel dostupný:
- Ve **3D pohledu** jako barevná linie s waypoint markery
- Na **2D mapě** (Topologie) jako projekce trasy
...
Délka se počítá automaticky ze 3D souřadnic včetně výškového rozdílu:
```
délka = √(vzdálenost_2D² + výškový_rozdíl²)
```
Přesnější než 2D měření — zahrnuje vertikální vedení kabelu.

### 28.7 Datový model route_3d

```json
[
  {"lng": 16.597, "lat": 49.189, "height_m": 0.5,
   "location_id": 42, "label": "Rack ServerovnaA"},
  {"lng": 16.597, "lat": 49.190, "height_m": 2.8,
   "location_id": 55, "label": "Krabice K1",
   "waypoint_type": "junction_box"},
  {"lng": 16.598, "lat": 49.190, "height_m": 7.2,
   "waypoint_type": "floor_penetration"}
]
```
...
| Endpoint | Metoda | Popis |
|---|---|---|
| `/topology/cables` | GET | Seznam kabelů (včetně route_3d) |
| `/topology/cables` | POST | Vytvoření kabelu |
| `/topology/cables/{id}` | PUT | Aktualizace kabelu |
| `/topology/cables/{id}` | DELETE | Smazání kabelu |
| `/buildings/{id}/3d` | GET | 3D data budovy (patra, zařízení, waypoints) |
