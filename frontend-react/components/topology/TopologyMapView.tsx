...
// Barvy dle hloubky
const DEPTH_COLORS = ["#3b82f6","#8b5cf6","#06b6d4","#10b981","#f59e0b","#ef4444"];
...
export default function TopologyMapView({
  ...
  onLocationMoved, onPolygonAction,
}: Props) {
  ...
  // Rozbalené podřízené lokace: Set id budov jejichž potomci jsou viditelní
  // Výchozí stav: regiony jsou vždy rozbaleny
  const [expandedLocs, setExpandedLocs] = useState<Set<number>>(() => {
    return new Set<number>();
  });

  // Auto-expand regionů při změně dat
  useEffect(() => {
  ...
  }, [locations.map(l => l.id).join(",")]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  useEffect(() => {
    ...
    clearDraw();
    const isDragMode = editMode === "drag_location";
    map.getContainer().style.cursor = editMode !== "none" && !isDragMode ? "crosshair" : "";
    setHint("");
    if (editMode === "none" || isDragMode) return;
    ...
    if (editMode === "draw_polygon") {
      setHint("Klikněte na marker budovy pro výběr.");
      map.on("click", (e: L.LeafletMouseEvent) => {
        if (!dr.polyLoc) {
          const pt = e.latlng;
          let closest: LocationMapPoint | null = null; let minDist = Infinity;
          ...
          if (!closest) { setHint("Klikněte blíže k markeru budovy."); return; }
          const loc = closest as LocationMapPoint;
          ...
          dr.polyMkrs.push(m);
          setHint(`Budova: ${loc.name} · Klikejte pro body · Dvojklik = uzavřít`);
          return;
        }
        const pt: [number, number] = [e.latlng.lat, e.latlng.lng];
  ...
  function undoLastPoint() {
    ...
    if (editMode === "draw_polygon" && dr.polyPts.length > 0) {
      const buildingMkrs = dr.polyLoc ? 1 : 0;
      if (dr.polyMkrs.length > buildingMkrs) { const last = dr.polyMkrs.pop(); if (last) map.removeLayer(last); }
  ...
  useEffect(() => {
    ...
    if (showPolygons) {
      buildingPolygons.forEach(bp => {
        ...
        // Bez permanentního popisku — info je dostupné přes klik nebo tooltip
    ...
    // Lokace
    if (showLocations) {
      // Které lokace zobrazit: top-level vždy + potomci rozbalených
      const visibleIds = new Set<number>();
      locations.forEach(loc => {
        if (loc.is_top_level) {
          visibleIds.add(loc.id);
        } else if (loc.parent_id && expandedLocs.has(loc.parent_id)) {
          visibleIds.add(loc.id);
          // Zobraz i potomky hlouběji (rekurzivně přes expandedLocs)
        }
      });
      // Rekurzivně přidej potomky expandovaných lokací na všech úrovních
      let changed = true;
      while (changed) {
        changed = false;
        locations.forEach(loc => {
          if (!visibleIds.has(loc.id) && loc.parent_id && visibleIds.has(loc.parent_id) && expandedLocs.has(loc.parent_id)) {
      ...
      locations.forEach(loc => {
        ...
        const isTopLevel = loc.is_top_level;
        const hasChildren = loc.children_count > 0;
        const isExpanded  = expandedLocs.has(loc.id);
        const color = depthColor(loc.depth);
        const radius = isBuilding ? 9 : isTopLevel ? 8 : Math.max(5, 7 - loc.depth);
        const hasOwnGps = loc.has_own_gps;

        // Marker — draggable v drag módu
        const marker = L.circleMarker([lat, lng], {
          radius, color, fillColor: color,
          fillOpacity: hasOwnGps ? 0.85 : 0.45,
          weight: isBuilding ? 2.5 : (hasOwnGps ? 2 : 1.5),
          dashArray: hasOwnGps ? undefined : "3 3",  // Přerušovaný = zděděné GPS
          bubblingMouseEvents: false,
        }).addTo(map);

        // Tooltip jen v drag módu (kde není klikací popup)
        if (isDragMode) {
          marker.bindTooltip(loc.name, { direction: "top", offset: [0, -radius - 2] });
        }

        // Badge pro počet potomků
        if (hasChildren) {
          const badge = L.marker([lat, lng], {
            icon: L.divIcon({
              ...
              html: `<div style="
                ...
                background:${isExpanded ? "#22c55e" : "#64748b"};
                border:1.5px solid white;
                display:flex;align-items:center;justify-content:center;
                font-size:9px;color:white;font-weight:700;
                cursor:pointer;
              ">${isExpanded ? "▲" : loc.children_count}</div>`,
        ...
        if (isDragMode) {
          ...
          draggable.on("dragend", async (e) => {
            ...
            try {
              await locationsGpsApi.updateGps(loc.id, pos.lat, pos.lng);
              onLocationMoved(loc, pos.lat, pos.lng);
            } catch (err) {
              alert("Chyba při ukládání souřadnic");
            }
          });

          layersRef.current.push(draggable);
          // Skryj původní circleMarker v drag módu
    ...
    if (showConnections) {
      connections.forEach(conn => {
        ...
        const latA = locA?.inherited_lat ?? locA?.lat;
        const lngA = locA?.inherited_lng ?? locA?.lng;
        const latB = locB?.inherited_lat ?? locB?.lat;
        const lngB = locB?.inherited_lng ?? locB?.lng;
    ...
    <div className="relative flex-1 h-full">
      ...
      <div className="absolute top-3 left-3 z-[1000] flex gap-1.5 flex-wrap">
        {!isDrawing && !isDragMode ? (
          ...
          <div className="flex items-center gap-2 bg-background/95 backdrop-blur-sm
                          border border-amber-400 rounded-lg shadow px-3 py-1.5">
            <Move className="h-3.5 w-3.5 text-amber-600" />
            <span className="text-xs font-medium text-amber-700 dark:text-amber-400">
              Přetahuj markery pro změnu GPS souřadnic
            </span>
          ...
          <div className="flex items-center gap-1.5 bg-background/95 backdrop-blur-sm
            ...
            <span className="text-xs font-medium text-amber-700 dark:text-amber-400">
              ...
              {isPolyDraw  && `🏢 Polygon${polyLoc ? `: ${polyLoc.name}` : ""}`}
            </span>
            {((isCableDraw && drawPoints.length > 0) || (isPolyDraw && polyPoints.length > 0)) && (
              <>
                ...
                <button onClick={undoLastPoint} className="p-1 text-muted-foreground hover:text-foreground" title="Zpět">
            ...
            {isPolyDraw && polyPoints.length >= 3 && (
              <button onClick={() => {
                const dr = drawRef.current;
                if (dr.polyLoc) { onBuildingPolygonDrawn(dr.polyLoc, [...dr.polyPts]); onEditModeChange("none"); }
              }}
                className="flex items-center gap-1 h-6 px-2 rounded bg-green-600 text-white text-xs font-medium hover:bg-green-700">
      ...
      {hint && isDrawing && (
        <div className="absolute bottom-16 left-1/2 -translate-x-1/2 z-[1000]
          ...
          {(isCableDraw || isPolyDraw) && (
            <span className="block text-muted-foreground mt-0.5">Dvojklik nebo Potvrdit/Dokončit = uzavřít</span>
          )}
        </div>
      )}
      ...
      {/* Legenda v drag módu */}
      {isDragMode && (
        <div className="absolute bottom-4 right-4 z-[1000] bg-background/90 border border-border
                        rounded-lg shadow px-3 py-2 text-xs space-y-1">
          <p className="font-medium text-foreground">Legenda GPS</p>
          <p className="text-muted-foreground">● Plný = vlastní GPS</p>
          <p className="text-muted-foreground">◌ Přerušovaný = zděděné GPS</p>
          <p className="text-muted-foreground text-[10px] mt-1">Přetáhni ⊕ marker pro uložení</p>
        </div>
      )}
        ...
        <div className="absolute bottom-4 left-4 z-[1000] bg-background/95 backdrop-blur-sm
          ...
          <div className="flex items-center justify-between mb-2">
            <span className="font-semibold text-foreground">
              ...
              {selected.type === "polygon"      && "🏢 Polygon budovy"}
              {selected.type === "building_loc" && "🏢 Budova"}
            ...
            <div className="space-y-2">
              ...
              {selected.data.children_count > 0 && (
                <button onClick={() => { toggleExpanded(selected.data.id); setSelected(null); }}
                  className="w-full flex items-center justify-between px-3 py-1.5 rounded-md bg-muted/50
          ...
          {selected.type === "location" && (
            <div className="space-y-1 text-xs text-muted-foreground">
              ...
              <p>Hloubka: {selected.data.depth}</p>
            ...
            <div className="space-y-2">
              <div className="text-xs">
                ...
                <p className="text-muted-foreground mt-1">
                  {selected.data.floor_count} {selected.data.floor_count === 1 ? "patro" : "patra"} · {selected.data.height_m} m
                  {selected.data.location_name && ` · ${selected.data.location_name}`}
                </p>
              </div>
              <div className="space-y-1.5 pt-1 border-t border-border">
                <button
                  onClick={() => {
                    onPolygonAction("view3d", selected.data);
                    setSelected(null);
                  }}
                ...
                <button
                  onClick={() => {
                    onPolygonAction("properties", selected.data);
                    setSelected(null);
                  }}
                  className="w-full flex items-center gap-2 px-3 py-2 rounded-md
                ...
                <button
                  onClick={() => {
                    onPolygonAction("edit_shape", selected.data);
                    setSelected(null);
                  }}
                  className="w-full flex items-center gap-2 px-3 py-2 rounded-md
                             bg-muted/50 text-foreground text-xs
                ...
                <button
                  ...
                  className="w-full flex items-center gap-2 px-3 py-2 rounded-md
                             text-destructive border border-destructive/30 text-xs
