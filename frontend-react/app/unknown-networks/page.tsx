...
function NetworkRow({ net, onAddRange }: {
  ...
  const { data: ips = [], isLoading } = useUnknownNetworkIps(open ? net.subnet : null) as { data: UnknownNetworkIp[]; isLoading: boolean };
    ...
    <div className="border border-border rounded-lg overflow-hidden mb-2">
      ...
      >
        ...
        <span className="text-sm text-muted-foreground w-24 shrink-0">
          {net.ip_count} IP {net.ip_count === 1 ? "adresa" : net.ip_count < 5 ? "adresy" : "adres"}
        </span>
                  ...
                  <tr key={ip.ip} className="border-b border-border/30 last:border-0 hover:bg-muted/20">
                    ...
                    <td className="px-3 py-1.5">
                      <div className="flex gap-1">
                        {(ip.sources as string[]).map(srcBadge)}
                      </div>
                    </td>
...
export default function UnknownNetworksPage() {
  const { data: networks = [], isLoading, refetch } = useUnknownNetworks();
  const qc = useQueryClient();

  function handleAddRange(subnet: string) {
    // Přesměrujeme na IP Rozsahy s předvyplněným CIDR
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="rounded-xl border border-border bg-card p-4">
          ...
          <p className="text-2xl font-bold mt-1 font-mono text-sm">
            {networks[0]?.subnet ?? "—"}
          </p>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="rounded-xl border border-border bg-card p-12 text-center">
          ...
          <p className="text-sm text-muted-foreground">Žádné neznámé sítě — všechny IP jsou v evidovaných rozsazích</p>
        </div>
      ) : (
        <div>
